...
const DEFAULT_API_BASE =
  'https://ttfpp-senior-experience-collector.elitetransportghana.workers.dev';
const API_BASE =
  typeof window.TTFPP_API_BASE === 'string' && window.TTFPP_API_BASE.trim().length > 0
    ? window.TTFPP_API_BASE.replace(/\/+$/, '')
    : DEFAULT_API_BASE;
...
async function startRecording(questionItem, textField, syncUi) {
  ...
  try {
    ...
    recorderState.detectedMimeType = mimeType || mediaRecorder.mimeType || '';
    ...
    mediaRecorder.ondataavailable = (event) => {
      if (event.data && event.data.size > 0) {
        ...
        if (event.data.type) {
          recorderState.detectedMimeType = event.data.type;
        }
      }
    };
    ...
    mediaRecorder.onstop = async () => {
      const blobType =
        recorderState.detectedMimeType ||
